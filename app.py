...
def preparar_dataframe(df_raw, col_data, col_valor, usar_valor_absoluto=False):
    ...
    Além do DataFrame limpo, devolve um diagnóstico linha a linha de tudo que
    foi descartado — número da linha na planilha original, campo problemático
    e valor bruto — para nunca depender de adivinhação quando algo é ignorado."""
    df = df_raw.copy()
    ...
    df_final["_data_original"] = df[col_data].values
    ...
    mascara_invalida = df_final["Data"].isna() | df_final["Valor"].isna()
    linhas_invalidas = int(mascara_invalida.sum())

    registros_diagnostico = []
    for _, linha in df_final[mascara_invalida].iterrows():
        motivos = []
        if pd.isna(linha["Data"]):
            motivos.append(f"Data não reconhecida (valor original: {linha['_data_original']!r})")
        if pd.isna(linha["Valor"]):
            motivos.append(f"Valor não reconhecido (valor original: {linha['_valor_original']!r})")
        registros_diagnostico.append({
            "Linha_na_planilha": int(linha["_linha_planilha"]),
            "Motivo": "; ".join(motivos),
        })
    diagnostico = pd.DataFrame(registros_diagnostico, columns=["Linha_na_planilha", "Motivo"])

    df_valido = df_final[~mascara_invalida].drop(
        columns=["_data_original", "_valor_original", "_linha_planilha"]
    ).copy()
...
def executar_conciliacao(df_banco, df_sistema, tolerancia_dias=0, progress_callback=None):
    """progress_callback (opcional): função chamada com um float 0.0-1.0
    conforme o cruzamento avança — usada para uma barra de progresso real,
    não decorativa, no app."""
    ...
    if tolerancia_dias > 0 and not apenas_banco.empty and not apenas_sistema.empty:
        ...
        for i, (idx_b, linha_b) in enumerate(apenas_banco.iterrows()):
...
def gerar_modelo_xlsx(tipo):
    ...
    referência de formato. Os dados dos dois modelos foram escolhidos para,
    se usados juntos, já demonstrarem na prática o match exato, o match por
    tolerância de dias e o match por valor absoluto."""
    if tipo == "banco":
        nome_aba = "Extrato Bancário"
        cor = "2F4B3C"
        df_exemplo = pd.DataFrame({
            "Data": pd.to_datetime(["2026-07-01", "2026-07-05", "2026-07-10", "2026-07-15", "2026-07-22"]),
            "Descrição": [
                "PAGAMENTO FORNECEDOR ABC", "RECEBIMENTO CLIENTE XYZ", "TARIFA BANCARIA",
                "PAGAMENTO FOLHA", "RENDIMENTO APLICACAO",
            ],
            "Valor": [-1500.00, 3000.00, -35.00, -8000.00, 45.20],
        })
    else:
        nome_aba = "Sistema"
        cor = "3A5A6B"
        df_exemplo = pd.DataFrame({
            "Data": pd.to_datetime(["2026-07-02", "2026-07-05", "2026-07-15", "2026-07-23"]),
            "Histórico": ["Fornecedor ABC", "Cliente XYZ", "Folha de pagamento", "Lançamento não identificado"],
            "Valor": [1500.00, 3000.00, 8000.00, 300.00],
        })

    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df_exemplo.to_excel(writer, sheet_name=nome_aba, index=False)
        _estilizar_planilha(writer.book[nome_aba], df_exemplo, cor_cabecalho=cor)
    return output.getvalue()
...
def calcular_preview(df_raw, col_data, col_valor):
    """Roda a mesma limpeza usada na conciliação, só que antes de o usuário
    clicar em processar — para confirmar visualmente que o mapeamento de
    colunas está correto e ver um resumo rápido do que foi lido."""
    datas = pd.to_datetime(df_raw[col_data], errors="coerce", dayfirst=True)
    valores = limpar_coluna_valores(df_raw[col_valor])
    datas_validas = datas.dropna()
    return {
        "total_linhas": len(df_raw),
        "linhas_problema": int((datas.isna() | valores.isna()).sum()),
        ...
        "qtd_creditos": int((valores > 0).sum()),
        "qtd_debitos": int((valores < 0).sum()),
    }
...
def main():
    ...
    with st.expander("📄 Não sabe como formatar sua planilha? Baixe um modelo"):
        st.caption("Use como referência de colunas — depois é só substituir pelos seus dados reais.")
        col_mod1, col_mod2 = st.columns(2)
        with col_mod1:
            st.download_button(
                "📥 Modelo de Extrato Bancário", data=gerar_modelo_xlsx("banco"),
                file_name="modelo_extrato_bancario.xlsx", mime=MIME_XLSX, use_container_width=True,
            )
        with col_mod2:
            st.download_button(
    ...
    for coluna, rotulo, df_raw_prev, prev in (
        (col_prev1, "Extrato Bancário", df_banco_raw, prev_banco),
        (col_prev2, "Sistema", df_sistema_raw, prev_sistema),
    ):
        with coluna:
            ...
            with pc1:
                cartao_metrica(
                    "Linhas lidas", prev["total_linhas"],
                    f"{prev['linhas_problema']} serão ignoradas" if prev["linhas_problema"] else "todas prontas",
                    "ambar" if prev["linhas_problema"] else "verde",
                )
            with pc2:
                if prev["data_min"] is not None:
                    periodo = f"{prev['data_min'].strftime('%d/%m/%y')}–{prev['data_max'].strftime('%d/%m/%y')}"
                else:
                    periodo = "—"
                cartao_metrica("Período", periodo, f"soma {formatar_moeda(prev['soma_valores'])}", "neutro")
            with st.expander("Ver primeiras linhas"):
                st.dataframe(df_raw_prev.head(5), use_container_width=True, hide_index=True)
    ...
    if st.button("🚀 Processar Conciliação", use_container_width=True, type="primary"):
        barra = st.progress(0, text="Iniciando conciliação...")
        try:
            barra.progress(10, text="Limpando dados do Extrato Bancário...")
            df_banco, invalidos_banco, diagnostico_banco = preparar_dataframe(df_banco_raw, col_data_banco, col_valor_banco, usar_abs)

            barra.progress(25, text="Limpando dados do Sistema...")
            df_sistema, invalidos_sistema, diagnostico_sistema = preparar_dataframe(df_sistema_raw, col_data_sistema, col_valor_sistema, usar_abs)
            ...
            def _atualizar_progresso_cruzamento(fracao):
                barra.progress(35 + int(fracao * 45), text=f"Cruzando lançamentos... {int(fracao * 100)}%")

            conciliados, divergencias = executar_conciliacao(
                df_banco, df_sistema, tolerancia_dias,
                progress_callback=_atualizar_progresso_cruzamento if tolerancia_dias > 0 else None,
            )

            barra.progress(85, text="Calculando estatísticas...")
            ...
            st.session_state.resultado = {
                ...
                "diagnostico_banco": diagnostico_banco, "diagnostico_sistema": diagnostico_sistema,
            }
    ...
    if r["invalidos_banco"] or r["invalidos_sistema"]:
        ...
        with st.expander("🔍 Ver exatamente quais linhas foram ignoradas e por quê"):
            if r["invalidos_banco"]:
                st.markdown("**Extrato Bancário**")
                st.dataframe(r["diagnostico_banco"], use_container_width=True, hide_index=True)
            if r["invalidos_sistema"]:
                st.markdown("**Sistema**")
                st.dataframe(r["diagnostico_sistema"], use_container_width=True, hide_index=True)
            st.caption("O número da linha considera o cabeçalho como linha 1 — bata com o mesmo número no Excel/Google Sheets.")
